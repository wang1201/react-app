  ...
  componentWillMount() {
    fetch('/mw/app/index/banner')
      ...
      .then(res => {
        // data就是我们请求的props
        this.setState({
          data: res.entity.indexCenterBanner,
        });
      });
  }
  // componentDidMount() {
            ...
            {this.state.data.map((val, index) => (
              <a
                ...
                style={{
                  ...
                  position: 'relative',
                  top: 10,
                  height: this.state.slideIndex === index ? 164 : 150,
                  marginTop: this.state.slideIndex === index ? 0 : 7,
                  marginBottom: this.state.slideIndex === index ? 0 : 7,
